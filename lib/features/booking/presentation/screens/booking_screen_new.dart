...
class _BookingScreenState extends ConsumerState<BookingScreen>
  ...
  void _showEditBookingDialog(Booking booking) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Edit booking dialog coming soon!')),
    );
  }
  ...
  void _showBookingDetailsDialog(Booking booking) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Booking Details - ${booking.bookingNumber}'),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow('Pet', booking.petName),
              _buildDetailRow('Room', booking.roomNumber),
              _buildDetailRow('Check-in', '${booking.checkInDate.toString().split(' ')[0]} at ${booking.checkInTime.hour}:${booking.checkInTime.minute.toString().padLeft(2, '0')}'),
              _buildDetailRow('Check-out', '${booking.checkOutDate.toString().split(' ')[0]} at ${booking.checkOutTime.hour}:${booking.checkOutTime.minute.toString().padLeft(2, '0')}'),
              _buildDetailRow('Status', booking.status.name.toUpperCase()),
              _buildDetailRow('Type', booking.type.name.toUpperCase()),
              _buildDetailRow('Total Amount', 'MYR ${booking.totalAmount.toStringAsFixed(2)}'),
